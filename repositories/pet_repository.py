...
class RepositorioPet:
    def cadastrar_pet(self, pet_dados, cliente_id: int):
        ...
        try:
            ...
            cursor.execute(sql, (pet_dados.nome, pet_dados.tipo, pet_dados.raca, pet_dados.idade, pet_dados.peso,
                                 pet_dados.sexo_biologico, pet_dados.observacoes, cliente_id))
            conn.commit()
    ...
    def buscar_consultas_por_pet_id(self, pet_id: int):
        ...
        try:
            conn = conectar()
            cursor = conn.cursor()
            # CORRIGIDO: Busca na tabela historico_medico onde o tipo é 'Consulta'
            sql = """
                  SELECT h.tipo_servico, f.nome, h.data_hora, h.valor
                  FROM historico_medico h
                           LEFT JOIN Funcionarios f ON h.funcionario_id = f.id
                  WHERE h.pet_id = %s \
                    AND h.tipo_servico = 'Consulta'
    ...
    def buscar_servicos_por_pet_id(self, pet_id: int):
        ...
        try:
            conn = conectar()
            cursor = conn.cursor()
            # CORRIGIDO: Busca na tabela historico_medico tudo que NÃO for 'Consulta'
            sql = """
                  SELECT h.tipo_servico, f.nome, h.data_hora, h.valor
                  FROM historico_medico h
                           LEFT JOIN Funcionarios f ON h.funcionario_id = f.id
    ...
    def transferir_pet(self, pet_id: int, novo_cliente_id: int):
        ...
        try:
            ...
            conn.commit()
            print(
                f"✅ TRANSFERÊNCIA CONCLUÍDA: Pet '{pet_nome}' transferido de {cliente_atual_id} para {novo_cliente_nome}")
    ...
    def excluir_pet(self, pet_id: int):
        """
