...
async def cadastrar_funcionario_admin(
    ...
    cargos_admin = [1, 2]  # IDs dos cargos administrativos

    if not token_data or user_type != "funcionario" or cargo_id not in cargos_admin:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Apenas administradores podem cadastrar funcionários.")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail=f"Erro ao cadastrar funcionário: {str(e)}")
...
async def rota_desativar_cliente(
    ...
    try:
        ...
        if cargo_id not in cargos_permitidos:
            ...
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="Apenas GESTORES podem alterar o status de clientes.")

        print(f"✅ Permissão concedida - é GESTOR (cargo_id: {cargo_id})")
        ...
        traceback.print_exc()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail=f"Erro ao alterar status do cliente: {str(e)}")